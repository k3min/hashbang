...
a.title;this.type=a.type;this.showTitle=a.showTitle;this.template=new HB.Template(this.type);for(var b in a.blocks)this.blocks[b]=new HB.Block(a.blocks[b])};b.HB.Collection.prototype.show=function(a){for(var b=this.title,c=this.type;HB.target.firstChild;)HB.target.removeChild(HB.target.firstChild);if(void 0===a||0===a.length){this.showTitle&&(HB.Collection.title.textContent=this.title,HB.target.appendChild(HB.Collection.title));for(var d in this.blocks)this.blocks[d].show()}else void 0!==this.blocks[a]?
(b=this.blocks[a].title,c=this.blocks[a].type,this.blocks[a].show()):location.hash="#!/{}".format(this.handle);document.title=HB.title.spec.format(HB.title.text,b);document.body.dataset.type=c};b.HB.Collection.title=document.createElement("h1");b.HB.Block=function(a){for(var b in a)this[b]=a[b];this.template=new HB.Template(this.type)};b.HB.Block.prototype.show=function(){HB.target.appendChild(this.template.render(this))};String.prototype.format=function(a){var b=0;a="string"===typeof a?Array.apply(null,
arguments):a;return this.replace(/:(\w+)|\{([0-9])?\}/g,function(c,d,e){return a[d||e||b++]})};b.addEventListener("load",HB.main.bind(HB),!1)})(this);